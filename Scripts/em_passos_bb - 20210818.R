...
######## --------------- Pivot to Wide format ----------------
  ...
  select(-row) %>%
  group_by(Partner, Province, District, Orgnunituid, fundingsource, NumDen, PatientType, KeyPop, FY, Date, Month) %>%
  summarise(HTS_TST = sum(HTS_TST), HTS_TST_POS = sum(HTS_TST_POS), KP_PREV = sum(KP_PREV), TX_NEW_VERIFY = sum(TX_NEW_VERIFY), TX_CURR_VERIFY = sum(TX_CURR_VERIFY), 
