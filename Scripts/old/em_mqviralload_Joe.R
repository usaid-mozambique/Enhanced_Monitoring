...
# DEFINE MONTH AND DEFINE PATHS -------------------------------------------

month <- "2021-10-20" # UPDATE EVERY MONTH

ajuda_path <- "~/GitHub/AJUDA_Site_Map/Dataout/ajuda_site_map_fy22q1.xlsx"
...
# DEFINE TIDY FUNCTION FOR MQ VL DATAFRAME ----------------------------------
  ...
  df <- read_excel(filename, sheet = "Monitoria Intensiva de CV", skip = 10) %>%
    ...
    mutate(idade = case_when(
      ...
      num_denom = case_when(
        ...
        str_detect(atributo, "13_iniciaram_2a_15+") ~ "D", # THIS CHUNK OF CODE ADDRESSES THE LACK OF D/N IDENTIFIERS IN THE DUPLICATE VARIABLE NAMES AND IN THE ONE OTHER
        str_detect(atributo, "duplicate") ~ "D"), 
      
      indicador = case_when(
        str_detect(atributo, "consulta_1a") ~ "pedido_1a",
        str_detect(atributo, "pedido_1a") ~ "pedido_1a",
        ...
        str_detect(atributo, "2ndpedido_1a") ~ "2ndpedidoaltocv_1a",
        str_detect(atributo, "2ndpedidoaltocv_1a") ~ "2ndpedidoaltocv_1a",
        ...
        str_detect(atributo, "duplicate") ~ "2ndpedidosupcv_1a",
        str_detect(atributo, "2ndpedidosupcv_1a") ~ "2ndpedidosupcv_1a",
