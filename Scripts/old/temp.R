...
path_df_sisma <- glue::glue("Data/MISAU/SMI/sisma_cpn_{year}.csv")


# FUNCTIONAL -----------------------------------------------------------

df <- clean_sisma_csv(path_df_sisma)

df_1 <- parse_sisma_smi_cpn(df)


unique(df_1$age)
unique(df_1$disaggregate)
list <- df_1 %>% distinct(indicator, age, disaggregate)
...
tribble_clean <- tribble %>% distinct(indicator, indicator_new, age, disaggregate)


write_excel_csv(tribble_clean, file = "sisma_smi_anc_indicator_map.csv")
