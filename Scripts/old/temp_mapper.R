...
path_df <- glue::glue("Data/MISAU/SMI/sisma_cpn_{year}.csv")
...
df_indicator_list <- df_parse %>% distinct(indicator, indicator_new, age, disaggregate)
write_excel_csv(df_indicator_list, file = "sisma_smi_cpn_indicator.csv")


df_test <- df %>% 
  left_join(df_indicator_list, by = "indicator")
